...
floor_class = "floor-pass" if run["meets_min_sample_floor"] else "floor-fail"
floor_text = "MEETS" if run["meets_min_sample_floor"] else "BELOW"

st.markdown(f"""
<div class="caveat-box">
  <b>{floor_text} the ~200-trades/12-month floor</b> for this period
  ({run['n_wins'] + run['n_losses']} decided trades over {(run['period_end'] - run['period_start']).days}d,
  {run['min_sample_floor_required']} required) — <span class="{floor_class}">{floor_text.lower()}</span>.
  This is a mechanism/metrics view, not an endorsement of edge — see the variant comparison below for the
  multiple-comparisons caveat, and treat the held-out test period as still within the same one-directional
  regime as the full period, not genuinely out-of-regime.
</div>
""", unsafe_allow_html=True)
