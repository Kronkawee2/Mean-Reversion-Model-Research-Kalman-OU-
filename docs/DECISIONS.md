...
## Entry-timing confirmation gate -- tested, NOT adopted (closes item #5 from the pending list)

Closes the long-pending item flagged earlier in this doc ("entry-timing empirical test: touch-immediate vs. wait-for-CHoCH-confirmation -- not yet done, was superseded... but not rejected"). Motivated by a real observed failure: a nested-chain LONG entered mid-decline on the raw touch bar's close, stopped out for -1R, with price reversing and running past target within the hour -- entries were firing on simple zone touch with no evidence the reversal had actually started.

**Built:** `composite_confluence_engine.apply_entry_confirmation()`. For each raw touch (`build_candidates()` output), searches forward bar-by-bar from the touch bar itself (a same-bar rejection counts, no artificial delay) for either (a) a closing-candle rejection -- a wick through the zone's FAR boundary followed by a close back inside/beyond it in the trade direction, i.e. a genuine sweep-and-reject, or (b) a CHoCH in the trade direction landing on that bar. Search window reuses `TOUCH_WINDOW_M15_BARS` (5h, same convention as `filter_roots_near_composite_factors`'s window reuse, no new constant invented) and is capped early at the zone's own `invalidated_at_bar` -- a zone that dies before confirming never gets a late confirmation from stale price action. No confirmation inside the window = candidate DISCARDED (missed entry), same skip-not-weaken convention as the LTF-drilling step and the TP1 R:R floor. Confirmed rows get `touch_idx`/`touch_time` reassigned to the confirmation bar, so entry price and every downstream factor score are evaluated as of the moment the trade would actually be taken. Comparison tooling: `scripts/backtest/compare_entry_confirmation.py`, matching baseline vs. confirmed signals by (direction, zone bounds) to isolate concrete before/after cases, run through the unmodified `structural_backtest_engine.simulate()` for real outcomes.

**Verified the mechanism does exactly what it was built for.** The motivating real trade (XAUUSD nested-chain bullish signal, entered 2026-08-06 19:45 at 4248.89 -- the close of an already-extended spike candle -- stopped at 4233.12 around 00:30-00:45 the next day, price then rallying to 4256+ within the hour) is correctly discarded by the gate: the zone invalidates before any rejection candle or CHoCH ever prints, so no confirmation ever arrives.

**Real-data result, 180-day nested window (matches `NESTED_WINDOW_DAYS`, the same bound production drilling itself uses), nested_chain entries only:**

| | XAUUSD baseline | XAUUSD confirmed | EURUSD baseline | EURUSD confirmed |
|---|---|---|---|---|
| Qualifying signals | 12 | 6 | 10 | 5 |
| Win rate | 91.7% | 100% | 80.0% | 80.0% |
| Expectancy | +2.548R | +3.749R | +2.847R | +1.639R |
| Total realized R | +30.58R | +22.49R | +28.47R | +8.20R |

Of the signals the gate discarded entirely (no confirmation ever arrived within the window): 7 of 8 on XAUUSD and 5 of 6 on EURUSD would have been WINNERS in baseline; only 1 per symbol would have been a loss.

**Rejected as a default -- net negative in aggregate despite working correctly on the case that motivated it.** Nested Zone Drilling's terminal zones are already M15/M5-tight by construction (the whole point of the hierarchy), so price very often reverses immediately at first touch without ever printing a distinct rejection candle or a fresh CHoCH before the zone itself invalidates. Gating on confirmation therefore mostly causes missed winners, not avoided losers, on this sample: total realized R dropped 26% (XAUUSD, +30.58R -> +22.49R) and 71% (EURUSD, +28.47R -> +8.20R). Per-trade win rate/expectancy look flat-to-better because the discarded pool skews winner-heavy, not because the gate is finding bad trades -- the aggregate number (total R, not the per-trade average) is what actually matters when comparing pools of very different size, and it moved the wrong way on both symbols. Sample size is small (n=10-12 qualifying signals per symbol) so this is not statistically conclusive, but the direction is consistent across both symbols tested.

**Kept as a tested, available, NOT-wired-in option** -- same treatment as `include_bos` and the `f_crt` scoring flag elsewhere in this module: `apply_entry_confirmation()` stays in `composite_confluence_engine.py`, unused by `run_composite_confluence_detection.py`, in case a differently-scoped version (e.g. a wider window, or gating only lower-score/higher-risk candidates rather than every touch) is worth testing later. Production entry mechanism is unchanged: raw touch-bar close, no confirmation gate.

## HTF Bias removed from composite score -- live-track-record isolation check, same failure mode as CRT

Prompted by a direct question ("HTF Bias has real effect? If it barely affects the main system, take it out?") during a dashboard-usability pass. Unlike every other factor-removal decision in this doc, this one was checked directly against the **accumulated live production track record** in `composite_confluence_signals` (real resolved trades produced by the adopted nested-chain mechanism) rather than a fresh backtest run, since 336 (XAUUSD) / 466 (EURUSD) real qualifying signals had already accumulated under the (then-current) 5-factor score.

**Method.** Isolated the subset of qualifying signals that ONLY cleared the >=3/5 threshold because of `f_bias` -- i.e. `score == 3 AND f_bias == 1`, meaning removing `f_bias` from the score would drop them below threshold -- and compared their resolved outcomes against every other qualifying signal (which would remain qualifying regardless of `f_bias`, since their other factors alone already summed to >= 3).

| | XAUUSD bias-dependent (score==3, f_bias==1) | XAUUSD everything else qualifying | EURUSD bias-dependent | EURUSD everything else qualifying |
|---|---|---|---|---|
| Resolved trades | 36 | 300 | 31 | 435 |
| Win rate | 25.0% (9/36) | 43.7% (131/300) | 19.4% (6/31) | 38.4% (167/435) |
| Expectancy | -0.157R | +0.579R | -0.287R | +0.426R |
| Total realized R | -5.66R | +173.60R | -8.91R | +185.17R |

Every signal that only qualified because HTF Bias agreed with the trade direction had **negative expectancy on both symbols**, while every other qualifying signal was solidly, consistently positive. This is a stronger and more direct signal than the original CRT-in-score rejection (that one was based on a fresh backtest sweep; this one is the actual accumulated live track record admitting money-losing trades it didn't need to admit).

**Decision.** Dropped `f_bias` from the score sum in `composite_confluence_engine.score_candidates()`, same treatment already established for `f_crt`: column still computed and persisted for telemetry, just excluded from `score`. `SCORE_THRESHOLD` left at the same numeric value (3) but is now out of 4 factors (`f_sweep`, `f_choch`, `f_zone_stack`, `f_div`) instead of 5 -- a stricter 75% bar instead of the prior 60%, and NOT re-derived by proportion the way the CRT-drop threshold was, because the correct new "qualifying" set (score>=3 under the new 4-factor sum) is provably a strict subset of the old qualifying set: `new_score = old_score - f_bias <= old_score`, so nothing that was previously rejected becomes newly admitted, and everything dropped is exactly the money-losing bias-dependent set measured above. Dashboard (`dashboard/pages/2_LTF_Triggers.py`) updated to match: `f_bias` removed from the "why this signal qualified" factor list (same treatment `f_crt` already had -- never shown, since neither contributes to score), and all "/5" score-display text changed to "/4".

**Not yet re-run through a fresh detection pass at the time of this decision** -- the analysis above is entirely a re-aggregation of already-stored per-factor columns on already-resolved signals (no new data needed, since `new_score <= old_score` guarantees no reclassification of the resolved sample), but the next full `run_composite_confluence_detection.py` pass will persist genuinely new rows under the 4-factor formula and should be spot-checked once it has accumulated enough resolved signals of its own.

## Volume Profile and HTF Bias removed from the automatic pipeline -- both are now dead ends

Direct follow-on from the `f_bias` scoring drop above, prompted by a user audit request ("is there anything else unused, like HTF Bias?"). Traced every consumer of every remaining `run_detection.py` stage's output across the whole codebase (production scripts, dashboard pages, archived pages) rather than guessing from the stage names.

**Findings, traced consumer-by-consumer:**
- `volume_profile` table: only ever read by `run_htf_bias_detection.py` (POC-alignment component of the HTF Bias score, cap ±10). Never read by `composite_confluence_engine.py`, never displayed on `dashboard/1_Chart.py` (checked directly -- no query against it there).
- `htf_bias` table (`bias`, `crt_equilibrium_bias`): only ever read by `run_composite_confluence_detection.py`, to populate `f_bias` and `f_crt` -- both now excluded from the composite score (see the entry above and the earlier CRT-drop entry). The one dashboard page that ever displayed HTF Bias (`2_HTF_Bias.py`) was already moved to `dashboard/_archived_pages/` in an earlier session (the user found the decomposed score breakdown unreadable and not useful day to day -- see the dashboard-simplicity lesson recorded separately). No other page or script reads this table for a live decision.
- `ema_20`/`ema_50`/`ema_200` (in `features`): only ever read by `run_htf_bias_detection.py`'s indicator-trend component. Not displayed on `dashboard/1_Chart.py` (checked -- only `atr_14` is read there). Left in place rather than dropped from the schema/feature-engineering script: same table also carries `atr_14`/`rsi_14`/`obv`/`stoch_k`/`stoch_d`/`cci_20`, which ARE still essential (ATR for stop/target sizing, the rest feeding Divergence detection), so removing just the EMA columns is a schema-level change for one dead field, not worth the risk for the marginal savings.
- Contrast case checked and confirmed still essential: `liquidity_sweeps` table. Composite Confluence recomputes sweeps fresh in-line (`LiquiditySweepStateEngine().detect_sweeps()` inside `run_composite_confluence_detection.py`'s `load_all()`) rather than reading this table -- but `dashboard/1_Chart.py` displays sweep markers straight from it, so the detection stage that populates it stays.

**Decision.** Removed the "Volume profile" and "HTF bias" stages from `run_detection.py`'s `build_stages()` -- both scripts (`run_volume_profile.py`, `run_htf_bias_detection.py`) are left in place and still independently runnable, just no longer invoked automatically by `run_detection.py` / `main.py` / the dashboard's "Run Pipeline" button. Updated the stage-order docstrings in `run_detection.py`, `main.py`, and `dashboard/pages/6_Run_Pipeline.py`, and the pipeline diagram in `CLAUDE.md`, to match. Saves roughly 20s per full pipeline run (small next to the ~900s nested-zone-drilling stage, but genuinely dead computation removed, not just low-impact).

**Consequence, accepted:** `f_bias`/`f_crt` telemetry columns on newly-generated `composite_confluence_signals` rows will read 0 going forward (the `htf_bias` table they're sourced from stops advancing past whenever this stage last ran) -- `score_candidates()` already defaults them to 0 when no `htf_bias` row exists for a given bar, so this is a graceful, no-crash degradation, not a break. If a future test wants to re-examine HTF Bias or Volume Profile with a different weighting/role, both stages can be re-added to `build_stages()` and backfilled with `--since` exactly as any other stage would be.

## Liquidity Sweep detection added to the same removal -- confirmed already dark on the chart, not just unread by scoring

Immediate follow-on question from the user on the entry above: "does the H1 sweep marker on the main chart actually match what f_sweep uses?" Checked directly (not guessed): `run_liquidity_sweep_detection.py` runs `LiquiditySweepStateEngine` against **H1** bars and persists to `liquidity_sweeps`; `run_composite_confluence_detection.py` runs the same engine class against **M15** bars, fresh, in-process, every pipeline pass -- never reading or writing the `liquidity_sweeps` table at all. These are two independently-computed, different-timeframe sweep sets sharing a name; the chart displaying one told the user nothing about the other.

Then checked whether the H1 set was even being displayed: `dashboard/1_Chart.py`'s Overlays sidebar hardcodes `show_crt = show_sweep = False` (dated 2026-08, when raw H1 overlays were replaced by the Nested Zone Chain view) -- `load_recent_sweeps()` was dead code, unreachable since that flag can never be set from the UI. So `liquidity_sweeps` (H1) had zero live consumers on either side: not read by scoring (recomputed fresh on M15 instead) and not read by display (toggle hardcoded off already).

**Decision.** Removed the "Liquidity sweeps" stage from `run_detection.py`'s `build_stages()` alongside Volume Profile and HTF Bias (same entry-point, `scripts/detection/run_liquidity_sweep_detection.py` still works standalone). Deleted the now-fully-dead `load_recent_sweeps()` function from `dashboard/1_Chart.py` and replaced its guarded call site with a plain `sweeps = []` -- zero behavior change (the toggle was already permanently off), just removed code that could never execute. Updated the stage-order text in `main.py`, `dashboard/pages/6_Run_Pipeline.py`, `run_detection.py`'s docstring, and the `CLAUDE.md` pipeline diagram to match. Pipeline is now: features -> SMC -> CRT -> divergence -> Composite Confluence -> resolution.
