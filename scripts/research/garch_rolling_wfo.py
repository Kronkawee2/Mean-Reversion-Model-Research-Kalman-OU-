"""
Rolling Walk-Forward Optimization for the GARCH(1,1)-filtered OU engine
(analysis/strategies/garch_ou_mean_reversion.py), mirroring
rolling_wfo.py/cir_rolling_wfo.py's methodology exactly (same fold-making,
same Bootstrap CI + Monte Carlo random-entry baseline) but pointed at
GARCH-OU instead of OU/CIR -- testing whether letting the band width
track volatility CLUSTERING (GARCH) does better than a level-dependent
band (CIR, RESULTS.md experiment 26-27: no combination passed both
significance checks) or a flat one (OU, experiment 19-22: nothing passed).

Usage:
    python scripts/research/garch_rolling_wfo.py --symbol XAUUSD --timeframe m5
"""
import argparse
import sys
import time
...
def run_cfg(dset, cost, **kw):
    res = run_garch_mean_reversion(dset["price_datetime"], dset["close_price"], dset["high_price"], dset["low_price"], **kw)
...
def build_grid():
    # Deliberately much smaller than OU/CIR's grid (24 vs 108 configs) and
    # recalib_every=20 instead of 5 -- GARCH(1,1) is fit by numerical MLE
    # (scipy.optimize) at every recalibration, ~1000x more expensive per
    # step than OU/CIR's closed-form moment estimators. At the full
    # 108-config/recalib_every=5 grid this would take single-digit HOURS
    # per fold; this coarser grid trades resolution for being able to
    # actually finish a multi-fold rolling WFO run.
    grid = []
    for calib_window in (60, 120):
        for k in (1.8, 2.2):
            for tau_frac in (1.0, None):
                grid.append(dict(
                    calib_window=calib_window, recalib_every=20,
                    obs_noise_scale=1.0, q_mult=1.0, k=k,
...
def main():
    ...
    print(f"=== {symbol} {table}: GARCH-OU Rolling WFO, train={args.train_days}d test={args.test_days}d step={step_days}d ===")
    ...
    out_path = ROOT / "scripts" / "research" / "plots" / "garch_ou" / f"garch_rolling_wfo_{symbol}_{table}.png"
    ...
    ax_eq.set_title(f"{symbol} {table} -- GARCH-OU Rolling WFO out-of-sample equity curve ({len(fold_records)} folds)")
