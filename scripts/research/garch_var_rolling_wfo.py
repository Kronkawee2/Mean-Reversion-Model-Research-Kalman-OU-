"""
Rolling Walk-Forward Optimization for the GARCH-OU engine with VaR
(empirical-quantile) entry/stop thresholds instead of k*sigma
(analysis/strategies/garch_ou_var_mean_reversion.py). Same methodology as
garch_rolling_wfo.py -- only the threshold definition being grid-searched
changes (entry_pctile/stop_pctile instead of k/z_stop).

Motivation: a naive entry_pctile=90/stop_pctile=99 (the literal numbers
from the pasted VaR-zone spec) produced 6,031 trades on XAUUSD M5 vs the
k=2.2 config's 104 -- far too loose a threshold, since k=2.2 under a
Normal approximation corresponds to roughly the 98.6th percentile, not
90th. This grid searches the percentile properly instead of assuming the
spec's example numbers were already tuned for this data.

Usage:
    python scripts/research/garch_var_rolling_wfo.py --symbol XAUUSD --timeframe m5
"""
import argparse
import sys
import time
...
from analysis.strategies.garch_ou_var_mean_reversion import run_garch_var_mean_reversion  # noqa: E402
from analysis.backtester.deflated_sharpe import trade_metrics  # noqa: E402


def run_cfg(dset, cost, **kw):
    res = run_garch_var_mean_reversion(dset["price_datetime"], dset["close_price"], dset["high_price"], dset["low_price"], **kw)
...
def build_grid():
    # calib_window fixed at 60 (the value config-stability found most
    # stable for GARCH-OU XAUUSD M5, RESULTS.md exp 30) -- this grid's
    # question is specifically "which percentile," not calib_window again.
    grid = []
    for entry_pctile in (95.0, 97.0, 98.0, 99.0):
        for stop_pctile in (99.0, 99.5):
            if stop_pctile <= entry_pctile:
                continue
            grid.append(dict(
                calib_window=60, recalib_every=20, q_mult=1.0, obs_noise_scale=1.0,
                entry_pctile=entry_pctile, stop_pctile=stop_pctile,
                half_life_mult=2.0, hmm_calib_bars=HMM_CALIB_BARS, hmm_block_states=(2,),
                tau_threshold=60, friction_hurdle_mult=2.5,
            ))
    return grid
...
def main():
    ...
    print(f"=== {symbol} {table}: GARCH-OU VaR-threshold Rolling WFO, train={args.train_days}d test={args.test_days}d step={step_days}d ===")
    ...
    for i, (tr_start, tr_end, te_end, train_df, test_df) in enumerate(folds):
        ...
        kw = res["best_kw"]
        print(f"  fold {i+1}/{len(folds)} [{tr_start.date()} -> {tr_end.date()} -> {te_end.date()}]: "
              f"entry={kw['entry_pctile']} stop={kw['stop_pctile']} train_pf={res['train_pf']:.2f} (n={res['train_n']}) -> "
    ...
    out_path = ROOT / "scripts" / "research" / "plots" / "garch_ou" / f"garch_var_rolling_wfo_{symbol}_{table}.png"
    ...
    ax_eq.set_title(f"{symbol} {table} -- GARCH-OU VaR-threshold Rolling WFO out-of-sample equity curve ({len(fold_records)} folds)")
