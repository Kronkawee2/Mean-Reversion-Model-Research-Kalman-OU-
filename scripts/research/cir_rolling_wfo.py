"""
Rolling Walk-Forward Optimization for the CIR mean-reversion engine
(analysis/strategies/cir_mean_reversion.py), mirroring rolling_wfo.py's
methodology exactly (same fold-making, same bootstrap significance check)
but pointed at CIR instead of OU -- testing whether letting the band width
scale with price level (rather than staying constant like OU) produces a
statistically significant edge where OU did not (see RESULTS.md
experiment 19-22: zero of five OU combinations passed).

Usage:
    python scripts/research/cir_rolling_wfo.py --symbol XAUUSD --timeframe m5
"""
import argparse
import sys
import time
...
from scripts.research.rolling_wfo import make_folds, bootstrap_expectancy_ci, monte_carlo_baseline  # noqa: E402,F401
...
def run_cfg(dset, cost, **kw):
    res = run_cir_mean_reversion(dset["price_datetime"], dset["close_price"], dset["high_price"], dset["low_price"], **kw)
    return sim_pnl(res) - cost


def extract_trades(dset, cost, **kw):
    """Same signal generation as run_cfg(), but also returns each trade's
    (entry_idx, exit_idx, net_pnl) -- the entry/exit bar indices are what
    the Monte Carlo random-entry baseline needs to draw a MATCHED holding-
    period distribution from (see monte_carlo_baseline() below)."""
    res = run_cir_mean_reversion(dset["price_datetime"], dset["close_price"], dset["high_price"], dset["low_price"], **kw)
    trades = []
...
def build_grid():
    grid = []
    for calib_window in (40, 60, 80, 120, 160, 200):
        for k in (1.8, 2.2):
            for q_mult, obs_noise_scale in ((1.0, 1.0), (2.0, 0.5), (1.0, 0.5)):
...
def main():
    ...
    print(f"=== {symbol} {table}: CIR Rolling WFO, train={args.train_days}d test={args.test_days}d step={step_days}d ===")
    ...
    print(f"  folds with PF>=1.0: {n_folds_profitable}/{len(pfs)} "
          f"({100*n_folds_profitable/len(pfs):.0f}%)")

    # Monte Carlo random-entry baseline: matched trade count + holding-
    # period distribution per fold, random direction/timing. Answers "is
    # this better than a strategy with zero skill but the same trade
    # frequency/duration profile," a stronger and more concrete benchmark
    # than just "CI clears zero."
    ...
    print(f"\n  Monte Carlo random-entry baseline (n_sims={n_sims}, matched trade count + holding-period "
          f"per fold, done in {time.time()-t_mc:.1f}s):")
    print(f"    real aggregate net PnL = {real_total:.2f}")
    print(f"    random baseline: mean={sim_totals.mean():.2f}  std={sim_totals.std():.2f}  "
          f"[5th,95th pct]=[{np.percentile(sim_totals,5):.2f}, {np.percentile(sim_totals,95):.2f}]")
    ...
    out_path = ROOT / "scripts" / "research" / "plots" / "cir" / f"cir_rolling_wfo_{symbol}_{table}.png"
    ...
    ax_eq.set_title(f"{symbol} {table} -- CIR Rolling WFO out-of-sample equity curve ({len(fold_records)} folds)")
